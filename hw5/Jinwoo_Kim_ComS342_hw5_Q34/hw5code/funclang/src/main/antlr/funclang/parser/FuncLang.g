...
import ListLang; //Import all rules from ListLang grammar.
 ...
 exp returns [Exp ast]:
        ...
        | ag=argexp { $ast = $ag.ast; }
        ;

 lambdaexp returns [LambdaExp ast]
        locals [ArrayList<Exp> formals]
 		@init { $formals = new ArrayList<Exp>(); } :
 		'(' Lambda
 			'(' ( tmp=argexp { $formals.add($tmp.ast); } )* ')'
 			body=exp
 ...
 argexp returns [ArgExp ast] :
 		'(' e1=Identifier
 		    Equal
 			e2=exp
 		')' { $ast = new ArgExp($e1.text, $e2.ast); }
 		| e1=Identifier { $ast = new ArgExp($e1.text); }
 		;